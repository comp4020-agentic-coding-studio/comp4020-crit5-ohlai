...
- **Point of view:** the wall is a physical surface being *shown* to you, not a
  ...
  **lights** a hold (lifted, haloed); the player **grips** one (full colour,
  ...
  the wall's move from their own falls without having made a mistake. A gripped
  hold then stays in colour for the rest of the round --- the route keeps a
  record of itself, so a long round asks the player to remember the route and
  not also their place in it.
- **One pitch per hold, in both voices.** The two voices are told apart by the
  eye, never by the ear. A hold's note is fixed by where it sits on the wall,
  and the wall showing it and the player pressing it must produce exactly that
  note --- the note is *how the player recognises the hold*, so a press that
  answers in a different pitch than the one it was shown in is not a second
  voice, it is the memory aid contradicting itself. Tried and reverted: a
  player pitch that rose with each press of a round. It made a clean round
  sound like a climb and made the game harder to actually play. Nothing in
  `check` catches this --- only listening does.
